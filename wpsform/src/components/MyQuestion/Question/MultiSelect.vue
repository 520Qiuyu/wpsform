  ...
  <div>
    ...
    <div class="problem-answer">
      <div
        ...
        :key="option.id"
      >
        <input type="checkbox" :checked="option.status === 1" disabled />
        <el-input
          v-model.lazy="option.title"
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    // 默认预选选项
    const defaultAnswer = ref(["请选择预选"]);
    // 设置题目预设
    const setDefaultAnswer = () => {
      thisProblem.value.setting!.options!.forEach((option: option) => {
        if (defaultAnswer.value.includes(option.id as string)) {
          option.status = 1;
        } else {
          option.status = 2;
        }
